...
function renderInline(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('*') && part.endsWith('*') && part.length > 2)
      return <em key={i} className="italic" style={{ color: 'var(--acc)' }}>{part.slice(1, -1)}</em>
    if (part.startsWith('`') && part.endsWith('`') && part.length > 2)
      return (
        <code key={i} className="font-mono rounded px-1.5 py-0.5"
          style={{ fontSize: '0.78em', background: 'var(--mid)', border: '1px solid var(--str)', color: 'var(--acc)' }}>
          {part.slice(1, -1)}
        </code>
      )
    return part
...
function MarkdownContent({ text }: { text: string }) {
  ...
  const elements = blocks.map((block, bi) => {
          ...
          {lines.map((line, li) => {
            ...
            return (
              <li key={li} className="flex gap-3 items-start">
                <span className="font-mono flex-none tabular-nums"
                  style={{ fontSize: '0.7rem', color: 'var(--fg3)', marginTop: 2, width: 16 }}>
                  {m[1]}.
                </span>
                <span className="flex-1 leading-relaxed">{renderInline(m[2])}</span>
              </li>
            )
          })}
        </ol>
          ...
          {lines.map((line, li) => {
            ...
            return content ? (
              <li key={li} className="flex gap-2 items-start">
                <span className="flex-none" style={{ color: 'var(--fg3)', marginTop: 4 }}>—</span>
    ...
    if (hm) {
      return <p key={bi} className="font-semibold text-[var(--fg)]">{renderInline(hm[1])}</p>
    }
  ...
  return <div className="space-y-3 text-sm text-[var(--fg)]">{elements}</div>
}
...
export default function OnboardingPortal() {
  ...
  // Mode detection from message content
  const detectMode = (text: string): AgentMode => {
  ...
  const send = useCallback(async (content: string) => {
    ...
    for (const f of files) {
      if (f.file.size > MAX_FILE_SIZE_BYTES) { alert(`"${f.file.name}" exceeds 10 MB limit`); return }
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body:    JSON.stringify({ message: text, messages: history, attachments }),
      ...
      // Stream response chunk by chunk (works for both Groq streaming and Gemini full response)
      ...
      setFiles(p => {
        p.forEach(f => { if (f.previewUrl) URL.revokeObjectURL(f.previewUrl) })
        return []
      })
      ...
      if (projectId && full) {
        fetch('/api/messages/save', {
          method:  'POST',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({ projectId, role: 'assistant', content: full, mode }),
        }).catch(() => {})
      }
    } catch (e) {
      ...
      setMessages(p => p.map(m => m.id === aid
        ? { ...m, content: 'Connection failed. Please check your internet and try again.' }
        : m
      ))
    } finally {
      setStreaming(false)
  ...
  }, [messages, projectId, streaming, phase, files])
  ...
  const addFiles = useCallback((raw: File[]) => {
    ...
    for (const f of raw) {
      const type = detectAssetType(f.type)
      if (!type) { alert(`"${f.name}" — unsupported file type`); continue }
      if (f.size > MAX_FILE_SIZE_BYTES) { alert(`"${f.name}" exceeds 10 MB`); continue }
      valid.push({
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-2xs font-mono tracking-[.20em] uppercase text-[var(--fg2)] select-none">
            OmniCraft Studios
          </span>
        </div>

        <div className="flex items-center gap-2 sm:gap-2.5">
          {/* Active mode pill — informational, not clickable */}
          <AnimatePresence mode="popLayout">
            {modes.slice(0, 1).map(m => {
              ...
              return (
                ...
                >
                  <motion.span
                    style={{ width: 5, height: 5, borderRadius: '50%', background: cfg.dot, display: 'block' }}
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain">
        <div
          className="max-w-chat mx-auto px-4 sm:px-6 py-8 sm:py-12"
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg, i) => (
              <motion.div
                ...
                transition={{ duration: .42, ease: E, delay: i === 0 ? .48 : 0 }}
            ...
            {phase === 'hold' && (
              ...
              >
                <HoldScreen
                  ...
                  projectSlug={projectSlug ?? projectId?.slice(0, 8) ?? ''}
                />
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="flex-none border-t border-[var(--str)] bg-[var(--bg)]/95 backdrop-blur-xl px-3 sm:px-6 pt-3 pb-safe">
        <div className="max-w-chat mx-auto">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: .35, duration: .5, ease: E }}
            className={cn(
              'rounded-2xl border transition-all duration-200 bg-[var(--in-bg)] border-[var(--in-str)]',
              locked
                ? 'opacity-40 pointer-events-none select-none'
                : 'focus-within:border-[var(--strS)] focus-within:shadow-[0_0_0_3px_var(--in-ring)]'
            )}
          >
            {/* File chips */}
            <AnimatePresence>
                    ...
                    >
                      {f.previewUrl
                        ...
                        : <span className="flex-none text-sm leading-none">
                            {f.type === 'pdf' ? '📄' : f.type === 'audio' ? '🎙' : f.type === 'video' ? '🎬' : '📎'}
                          </span>
                      }
                      ...
                      <button
                        ...
                        className="flex-none text-[var(--fg3)] hover:text-[var(--fg2)] transition-colors ml-0.5"
            ...
            <div className="flex items-end gap-1 px-2 py-2 sm:py-2.5">
              ...
              <textarea
                ...
                disabled={locked}
                placeholder={
                  phase === 'hold' ? 'Your session is saved.' :
              ...
              >
                {streaming
                  ? <motion.span className="block w-3 h-3 rounded-full border-[1.5px] border-[var(--fg3)] border-t-[var(--fg)]"
                      animate={{ rotate: 360 }} transition={{ duration: .75, repeat: Infinity, ease: 'linear' }} />
                  : <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                      <path d="M6 11V1M6 1L2 5M6 1l4 4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                }
              </motion.button>
            </div>
          ...
          <p className="hidden sm:block text-center text-2xs text-[var(--fg3)] mt-2 select-none">
            {phase === 'discovery'
              ? 'Return to send · Shift+Return for new line · Attach images, PDFs, audio & video'
              : phase === 'gate'
              ? 'Complete the form above to save your roadmap'
              : 'Submitted — check back for your roadmap'}
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-[90vw] sm:max-w-[580px] w-full">
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="w-[18px] h-[18px] rounded-[4px] bg-[var(--fg)] flex items-center justify-center flex-none">
          <div className="w-[7px] h-[7px] rounded-[2px] bg-[var(--bg)]" />
        </div>
        <span className="text-2xs font-mono text-[var(--fg3)] uppercase tracking-[.16em]">
          OmniCraft Studios
        </span>
      </div>
      ...
      {isEmpty && typing ? (
        ...
        <div className="relative">
          ...
          {typing && msg.content && (
            <motion.span
              className="inline-block w-0.5 h-[13px] bg-[var(--fg)] ml-px align-middle rounded-full"
              animate={{ opacity: [1, 0] }}
  ...
  return (
    <div className="flex items-center gap-1.5 py-1 px-1">
      {[0, 1, 2].map(i => (
        <motion.span
          key={i}
          className="block w-1 h-1 rounded-full bg-[var(--fg3)]"
          animate={{ opacity: [.2, 1, .2] }}
          transition={{ duration: 1.1, repeat: Infinity, delay: i * .18 }}
        />
      ))}
    </div>
  )
...
function GateForm({ projectId, onComplete }: { projectId: string; onComplete: (d: GateFormData) => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const endpoint = projectId === 'pending' ? null : '/api/gate'
      if (endpoint) {
      ...
      setErrs({ email: 'Something went wrong. Please try again.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[var(--fg2)] leading-[1.85] mb-7">
        Leave your details and the Studio will return with a finalised scope and quote within 24–48 hours.
      </p>
      ...
      <form onSubmit={submit} className="space-y-4" noValidate>
        {fields.map((f, i) => (
          <motion.div key={f.k} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i*.06+.1, duration: .3 }}>
            ...
            <input
              type={f.t}
              value={form[f.k] ?? ''}
              onChange={e => { setForm(p => ({ ...p, [f.k]: e.target.value })); setErrs(p => ({ ...p, [f.k]: undefined })) }}
              placeholder={f.ph}
              className={cn(
                'w-full rounded-xl px-4 text-sm text-[var(--fg)] bg-[var(--in-bg)] border placeholder:text-[var(--fg3)] focus:outline-none transition-all min-h-[50px]',
                errs[f.k]
                  ? 'border-red-500/50 bg-red-500/5'
                  : 'border-[var(--in-str)] focus:border-[var(--strS)] focus:shadow-[0_0_0_3px_var(--in-ring)]'
              )}
              style={{ fontSize: '16px' }}
            />
            <AnimatePresence>
              {errs[f.k] && (
                <motion.p initial={{ opacity: 0, y: -3, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }} exit={{ opacity: 0 }}
                  className="text-2xs text-red-400 mt-1 font-mono overflow-hidden">
                  {errs[f.k]}
                </motion.p>
              )}
            </AnimatePresence>
        ...
        <motion.button
          ...
          className={cn(
            'w-full rounded-xl text-sm font-medium tracking-wide min-h-[52px] mt-1 transition-all',
            loading
              ? 'bg-[var(--mid)] border border-[var(--str)] text-[var(--fg2)] cursor-wait'
              : 'bg-[var(--fg)] text-[var(--bg)] hover:opacity-90 active:scale-[.98]'
          )}
        >
          {loading
            ? <span className="flex items-center justify-center gap-2">
                <motion.span className="block w-3.5 h-3.5 rounded-full border-[1.5px] border-[var(--fg2)] border-t-transparent"
                  animate={{ rotate: 360 }} transition={{ duration: .75, repeat: Infinity, ease: 'linear' }} />
                Saving…
              </span>
            : 'Save My Roadmap →'
        ...
        <p className="text-2xs text-[var(--fg3)] text-center pb-1">
          Your details are used solely to deliver your project roadmap.
        </p>
      </form>
    </motion.div>
  )
    ...
    >
      ...
      <div className="flex items-center gap-2.5 mb-6">
        <span className="relative flex h-2 w-2 flex-none">
          <motion.span className="absolute inset-0 rounded-full bg-amber-400"
            animate={{ scale: [1,2.8,1], opacity: [.8,0,.8] }} transition={{ duration: 2.2, repeat: Infinity }} />
      ...
      <div className="space-y-3 text-sm leading-[1.85] mb-7">
        <p className="text-[var(--fg)]">
          {clientName ? `Thank you, ${clientName.split(' ')[0]}.` : "You're in."}{' '}
          Everything we've discussed is saved to your private dashboard.
        </p>
        <p className="text-[var(--fg2)]">
          The Studio Owner is reviewing the AI-generated roadmap to validate scope and finalise pricing. Expect to hear back within 24–48 hours.
        </p>
      </div>
      ...
      <div className="bg-[var(--surf)] border border-[var(--str)] rounded-2xl p-4 sm:p-5">
        ...
        <a href={url} target="_blank" rel="noopener noreferrer"
          className="text-xs font-mono text-[var(--fg2)] break-all hover:text-[var(--fg)] transition-colors">
          {url}
        </a>
        <p className="text-2xs text-[var(--fg3)] mt-3">
          Bookmark this link. Your roadmap and quote appear here once published.
        </p>
      </div>
    </motion.div>
  )
