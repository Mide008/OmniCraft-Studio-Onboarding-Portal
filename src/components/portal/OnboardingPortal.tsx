...
const GATE_TRIGGER = 10             // 10 AI responses for a full conversation
const SESSION_KEY = 'oc-session'   // localStorage key for returning users
    ...
    <div className="space-y-3 text-sm text-[var(--fg)]">
      {blocks.map((block,bi)=>{
        ...
        if(lines.some(l=>/^\d+[.)]\s/.test(l.trim()))) {
          return <ol key={bi} className="space-y-2">{lines.map((line,li)=>{
        ...
        if(lines.some(l=>/^[-*•]\s/.test(l.trim()))) {
          return <ul key={bi} className="space-y-1.5">{lines.map((line,li)=>{
            const content=line.trim().replace(/^[-*•]\s+/,'')
            return content?<li key={li} className="flex gap-2 items-start"><span style={{color:'var(--fg3)',marginTop:4,flexShrink:0}}>—</span><span>{renderInline(content)}</span></li>:null
          })}</ul>
        }
...
export default function OnboardingPortal() {
  ...
  // ── Init: check for returning session ─────────────────────────────────────
  useEffect(() => {
    ...
    if (storedSlug) {
      ...
      fetch(`/api/session?slug=${storedSlug}`)
        .then(r => r.json())
        .then(data => {
          if (data.projectId && data.canContinue !== false) {
            ...
            if (restored.length >= GATE_TRIGGER) gatedRef.current = true
          } else {
            setMessages([WELCOME])
          }
          setReady(true)
  ...
  // ── Persist session slug to localStorage ──────────────────────────────────
  useEffect(() => {
    if (projectSlug) localStorage.setItem(SESSION_KEY, projectSlug)
  }, [projectSlug])
  ...
  const send=useCallback(async(content:string)=>{
    ...
    for(const f of files){
      if(f.file.size>MAX_FILE){alert(`"${f.file.name}" exceeds 10 MB`);return}
      const base64=await fileToB64(f.file)
      attachments.push({name:f.file.name,mimeType:f.file.type,base64})
    }

    const userContent=text||(attachments.length>0?`[Attached ${attachments.length} file(s)]`:'')
    ...
    if(isFirst){
      fetch('/api/analytics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'first_message_sent',projectId})}).catch(()=>{})
    }
    if(attachments.length>0){
      fetch('/api/analytics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'file_attached',projectId,metadata:{count:attachments.length}})}).catch(()=>{})
    }
    ...
    try{
      ...
      if(mbHeader){ try{ setMoodboard(JSON.parse(mbHeader)) }catch{} }
  ...
  const addFiles=useCallback((raw:File[])=>{
    ...
    for(const f of raw){
      const type=detectAssetType(f.type); if(!type){alert(`"${f.name}" — unsupported type`);continue}
      if(f.size>MAX_FILE){alert(`"${f.name}" exceeds 10 MB`);continue}
  ...
  const handleHumanReview=async()=>{
    if(!projectId||!clientName) return
    setReviewSent(true)
    await fetch('/api/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,name:clientName,email:'',message:reviewMsg})})
  ...
  const startNewSession=()=>{
    ...
    setIsReturning(false); gatedRef.current=false
    ...
    <div className="flex flex-col bg-[var(--bg)] text-[var(--fg)] font-sans" style={{height:'100dvh',overflow:'hidden'}}>
      ...
      <header className="flex-none flex items-center justify-between px-4 sm:px-6 border-b border-[var(--str)] bg-[var(--bg)]/90 backdrop-blur-xl" style={{height:52}}>
        <div className="flex items-center gap-2.5">
          ...
          {isReturning&&(
            <motion.span initial={{opacity:0,x:-6}} animate={{opacity:1,x:0}} className="text-2xs font-mono text-emerald-400 hidden sm:block">· Returning</motion.span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          {/* New session button for returning users */}
          {(isReturning||phase==='hold') && (
            <button onClick={startNewSession} title="Start a new project"
        ...
        <div className="max-w-chat mx-auto px-4 sm:px-6 py-8 sm:py-12" style={{display:'flex',flexDirection:'column',gap:28}}>
          ...
          {isReturning&&ready&&(
            <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className="bg-emerald-950/30 border border-emerald-800/40 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
              ...
              <button onClick={startNewSession} className="text-2xs font-mono text-emerald-400 hover:text-emerald-300 transition-colors flex-none">Start new →</button>
            </motion.div>
          )}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {moodboard.colors.map((hex,i)=>(
                        <div key={i} className="flex items-center gap-1.5 text-2xs font-mono">
                          <span className="w-5 h-5 rounded border border-[var(--str)]" style={{background:hex}}/>
            ...
            {showReview&&(
              <motion.div key="review-modal" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                ...
                <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:20}} transition={{ease:E,duration:.35}}
                  className="w-full max-w-md bg-[var(--surf)] border border-[var(--str)] rounded-2xl p-6 shadow-[var(--sh)]">
                  ...
                  <div className="flex gap-3">
                    <button onClick={()=>setShowReview(false)} className="flex-1 border border-[var(--str)] rounded-xl py-2.5 text-sm text-[var(--fg2)] hover:text-[var(--fg)] transition-colors">Cancel</button>
                    <button onClick={handleHumanReview} className="flex-1 bg-[var(--fg)] text-[var(--bg)] rounded-xl py-2.5 text-sm font-medium hover:opacity-90 transition-all">Send Request</button>
                  </div>
                </motion.div>
              </motion.div>
            )}
        ...
        <div className="max-w-chat mx-auto">
          <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:.35,duration:.5,ease:E}}
                ...
                <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} className="flex flex-wrap gap-1.5 px-3 pt-2.5">
                  {files.map(f=>(
                    <motion.div key={f.id} initial={{opacity:0,scale:.85}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:.85}}
                      ...
                      <button onClick={()=>{if(f.previewUrl)URL.revokeObjectURL(f.previewUrl);setFiles(p=>p.filter(x=>x.id!==f.id))}} className="flex-none text-[var(--fg3)] hover:text-[var(--fg2)] transition-colors ml-0.5">×</button>
                    </motion.div>
                  ))}
                </motion.div>
              )}
            ...
            <div className="flex items-end gap-1 px-2 py-2 sm:py-2.5">
              <button onClick={()=>fileRef.current?.click()} disabled={locked} title="Attach file (PDF, image, audio, video)"
              ...
              <textarea ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} disabled={locked}
                placeholder={phase==='hold'?'Your session is saved — the Studio will reach out shortly.':phase==='gate'?'Fill in your details above…':streaming?'Thinking…':'Describe your project, share a reference…'}
          ...
          <p className="hidden sm:block text-center text-2xs text-[var(--fg3)] mt-2 select-none">
            {phase==='discovery'?'Return to send · Shift+Return for new line · Attach images, PDFs, audio & video':phase==='gate'?'Complete the form above to save your roadmap':'Submitted — your dashboard shows your roadmap once published'}
          </p>
        </div>
      </div>
    </div>
        ...
        {fields.map((f,i)=>(
          <motion.div key={f.k} initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} transition={{delay:i*.06+.1,duration:.3}}>
            ...
            <input type={f.t} value={form[f.k]??''} onChange={e=>{setForm(p=>({...p,[f.k]:e.target.value}));setErrs(p=>({...p,[f.k]:undefined}))}} placeholder={f.ph}
              className={cn('w-full rounded-xl px-4 text-sm text-[var(--fg)] bg-[var(--in-bg)] border placeholder:text-[var(--fg3)] focus:outline-none transition-all min-h-[50px]',errs[f.k]?'border-red-500/50 bg-red-500/5':'border-[var(--in-str)] focus:border-[var(--strS)] focus:shadow-[0_0_0_3px_var(--in-ring)]')} style={{fontSize:'16px'}}/>
...
function HoldScreen({clientName,projectSlug,onRequestReview,reviewSent}:{clientName?:string;projectSlug:string;onRequestReview:()=>void;reviewSent:boolean}){
  ...
  return(
    <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:.5,ease:[.22,1,.36,1]}} className="w-full max-w-[92vw] sm:max-w-md">
      ...
      {/* Human review button */}
      {!reviewSent?(
        <button onClick={onRequestReview} className="w-full border border-[var(--str)] rounded-xl py-3 text-sm text-[var(--fg2)] hover:text-[var(--fg)] hover:border-[var(--strS)] transition-all">
